...
class HomeChart
{
    ...
    private static function SUM_DATA($query, $field)
    {
        ...
        foreach (self::DATE_RANGE() as $date) {
            ...
            switch (self::$period) {
                case 'weekly':
                    $builder->whereBetween('created_at_232187', [Carbon::parse($date), Carbon::parse($date)->endOfWeek()]);
                    $label = Carbon::parse($date)->format('W Y');
                    break;
                case 'monthly':
                    $builder->whereBetween('created_at_232187', [Carbon::parse($date), Carbon::parse($date)->endOfMonth()]);
                    $label = Carbon::parse($date)->translatedFormat('F Y');
                    break;
                case 'yearly':
                    $builder->whereBetween('created_at_232187', [Carbon::parse($date), Carbon::parse($date)->endOfYear()]);
                    $label = Carbon::parse($date)->format('Y');
                    break;
                default:
                    $builder->whereDate('created_at_232187', $date);
